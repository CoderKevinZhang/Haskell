...
isBST (Leaf i) = True
isBST (Node i l r)
    | i < (head (preorder l)) = False
    | i > (head (preorder r)) = False
    | isBST l == False = False
    | isBST r == False = False
    | otherwise = True
...
compress (x:xs) = compress' 1 x xs where
    compress' i x [] = [(i,x)]
    compress' i x (y:ys)
        | x == y = compress' (i+1) x ys
        | otherwise = (i,x) : compress' 1 y ys
...
decompress (x:xs) = 
        if (null xs) then
            if ((fst x)==1) then [snd x]
            else [snd x] ++ decompress [(fst x-1,snd x)]
        else decompress [x] ++ decompress xs 
